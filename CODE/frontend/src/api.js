export const API_BASE = import.meta.env.VITE_API_BASE || "http://192.168.1.124:3000/api";
...
export const dashboard=(role,token)=>{ const path=role==="USER"?"user":role==="ORGANIZER"?"organizer":role==="ADMINISTRATOR"?"admin":null; if(!path) throw new Error("Không xác định được quyền tài khoản. Vui lòng đăng xuất và đăng nhập lại."); return api(`/dashboard/${path}`,{token}); };
