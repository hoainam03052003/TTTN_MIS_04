...
async function createEvent(req, res) {
    ...
    try {
        ...
        if ((checkin_start && !checkin_end) || (!checkin_start && checkin_end)) {
            return res.status(400).json({
                success: false,
                message: "Phải nhập đủ thời gian bắt đầu và kết thúc Check-in"
            });
        }

        if (checkin_start && checkin_end && new Date(checkin_end) <= new Date(checkin_start)) {
            return res.status(400).json({
                success: false,
...
async function getEvents(req, res) {
    ...
    try {
        ...
        const events =
            await eventService.getEvents(req.user);
...
async function getEventById(req, res) {
    ...
    try {
        ...
        if (req.user.role === "USER" && event.status !== "PUBLISHED") {
            return res.status(404).json({ success: false, message: "Không tìm thấy Event" });
        }

        if (req.user.role === "ORGANIZER" && Number(event.organizer_id) !== Number(req.user.userId)) {
            return res.status(403).json({
                success: false,
                message: "Bạn không có quyền xem Event này"
            });
        }
...
async function getEventTypes(req, res) {
    try {
        ...
        console.error(error);
        return res.status(500).json({ success: false, message: "Không thể lấy loại Event" });
    }
}
